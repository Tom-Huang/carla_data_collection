...
def find_seg_data_path(foldername):
    ...
    data_paths = []
    data_paths.append(foldername + "/" +
                      dir_list[0] + "/" + "left/CameraSemSeg/")
    data_paths.append(foldername + "/" +
                      dir_list[0] + "/" + "right/CameraSemSeg/")
    data_paths.append(foldername + "/" +
                      dir_list[0] + "/" + "front/CameraSemSeg/")
    data_paths.append(foldername + "/" +
                      dir_list[0] + "/" + "rear/CameraSemSeg/")
    return data_paths
...
def save_images(data_path, data_filename, save_path):
    ...
    for i, image in enumerate(images):
        ...
        if save_path[-1] != "/":
            save_path = save_path + "/"
        img_pil.save(save_path + data_filename.rstrip(".npy") +
                     str(i) + ".png")
